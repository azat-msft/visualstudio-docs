...
|**/InIsolation**|Runs the tests in an isolated process.<br />This isolation makes the *vstest.console.exe* process less likely to be stopped on an error in the tests, but tests might run slower.<br />By default, a non-parallel .NET Framework run that doesn't use data collectors can run inside the *vstest.console.exe* process; this option forces those tests into a separate process. .NET (`dotnet vstest`) tests always run in a separate process regardless of this option.|
...
Supported verbosity levels include quiet, minimal, normal, and detailed. When you don't specify a verbosity level, the console logger defaults to `normal` for *vstest.console.exe*, and to `minimal` for `dotnet vstest`.
